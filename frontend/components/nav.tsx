...
export default function Nav() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50 px-4 pt-4 md:px-8 md:pt-6">
      <motion.nav
        initial={{ opacity: 0, y: -8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, ease: 'easeOut', delay: 0.05 }}
        className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between gap-2 rounded-full bg-invert pl-4 pr-2 shadow-[0_10px_40px_rgba(0,0,0,0.45)] md:h-[62px] md:pl-6 md:pr-3"
      >
        <Link href="/" aria-label="Momentum home" className="flex items-center gap-3 shrink-0">
          ...
          />
          <span className="font-mono text-[18px] font-semibold uppercase tracking-[0.1em] text-invert-ink">
            Momentum
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map(({ label, href }) => {
            const cls =
              'font-mono text-[15px] text-invert-ink-2 hover:text-invert-ink transition-colors'
        ...
        {username ? (
          ...
          <span aria-hidden className="h-12 w-36 shrink-0 rounded-full bg-page/30" />
        )}
        </div>
      </motion.nav>
    </div>
