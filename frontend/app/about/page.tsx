...
export default function AboutPage() {
  return (
    <main className="min-h-screen bg-page text-ink-2 flex flex-col">
      ...
      {/* Hero */}
      <section className="mx-auto w-full max-w-[1500px] px-6 pt-40 pb-20">
        <p className="text-[10px] font-mono text-ink-4 uppercase tracking-[0.18em] mb-6">
          About
        </p>
        <h1
          className="text-[64px] md:text-[80px] font-normal leading-[1.05] tracking-tight text-ink max-w-3xl"
        ...
        <p className="mt-6 text-[17px] leading-relaxed text-ink-3 max-w-xl">
      ...
      {/* Divider */}
      <div className="border-t border-line mx-6 md:mx-20" />
      ...
      <section className="mx-auto w-full max-w-[1500px] px-6 py-20">
        <div className="grid md:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <h2
              className="text-4xl font-normal text-ink leading-snug"
              style={{ fontFamily: 'var(--font-display)' }}
            >
              make your output undeniable
            </h2>
          </div>
          <div className="space-y-5 text-[15px] leading-relaxed text-ink-3">
            <p>
      ...
      {/* Values */}
      <section className="mx-auto w-full max-w-[1500px] px-6 py-20">
        <p className="text-[10px] font-mono text-ink-4 uppercase tracking-[0.18em] mb-12">
          Values
        </p>
        <div className="grid md:grid-cols-3 gap-px bg-line">
          {values.map((v) => (
            <div key={v.label} className="bg-page p-8">
              <p className="text-ink text-sm font-semibold mb-3">{v.label}</p>
              <p className="text-ink-3 text-sm leading-relaxed">{v.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-[1500px] px-6 py-20">
        ...
        <div className="space-y-0">
          {timeline.map((item, i) => (
            <div
              key={i}
              className="grid md:grid-cols-[160px_1fr] gap-4 border-t border-line py-7"
            >
              <p className="text-ink-4 font-mono text-sm pt-0.5">{item.year}</p>
              <p className="text-ink-3 text-sm leading-relaxed">{item.event}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-[1500px] px-6 py-24">
        <h2
          className="text-4xl md:text-5xl font-normal text-ink leading-snug mb-8"
          style={{ fontFamily: 'var(--font-display)' }}
        >
        ...
        <div className="flex items-center gap-4">
          <Link
            href="/signup"
            className="bg-invert text-invert-ink text-sm font-semibold px-5 py-2.5 rounded-md hover:bg-invert/90 transition-colors"
          >
            Get started free
          </Link>
          <Link
